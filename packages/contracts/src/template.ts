...
export type TableColumnVisibilityScope='DOCUMENT'|'ANY_ROW'|'ALL_ROWS';
export type TableColumnLayoutRole='CONTENT'|'SPACER';
export interface TableColumnDefinition { id:string; label:string; path:string; layoutRole?:TableColumnLayoutRole; kind?:TableColumnKind; /** Original imported source header used as a safe fallback when the mapped path is not present in an item row. */ sourceField?:string; targetPath?:string; widthPercent?:number; alignment?:Alignment; headerAlignment?:Alignment; headerStyle?:TextStyle; cellStyle?:TextStyle; format?:DisplayFormatDefinition; visibility?:VisibilityRule; /** UX-8.4: whether visibility is evaluated once per document or across filtered table rows. */ visibilityScope?:TableColumnVisibilityScope; formulaExpression?:string; formulaBindings?:FormulaFieldBinding[]; staticValue?:string|number; imageWidthMm?:number; imageHeightMm?:number; qr?:QrCodeOptions; /** Optional footer behavior used by the designer to build/update total rows. Renderers still receive only resolved footer values. */ footerAggregation?:FooterAggregationType; footerCustomLabel?:string; }
