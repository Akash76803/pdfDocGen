...
function camelSegment(raw) {
    ...
    if (/^[0-9]/.test(value))
        value = `field${value.charAt(0).toUpperCase()}${value.slice(1)}`;
    if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(value) || BLOCKED.has(value))
        value = `field${value.charAt(0).toUpperCase()}${value.slice(1)}`;
    return value || 'field';
}
/**
 ...
 */
export function toApiSafePath(path) {
    const raw = path.trim();
    if (!raw)
        return '';
