...
import { Archive, Clock3, Copy, FileText, Layers3, Plus, Search, Send, Trash2 } from 'lucide-react';
...
export function Templates({ onNavigate }: { onNavigate: (route: AppRoute) => void }) {
  ...
  const filtered = useMemo(() => {
    ...
    return templates.filter((item) => {
      const matchesSearch = !term || item.name.toLowerCase().includes(term) || item.documentType.toLowerCase().includes(term);
  ...
  return <div className="page">
    ...
    <PageHeader eyebrow="Library" title="Templates" description="Reusable layouts for invoices, quotations, reports, certificates, agreements and letters." actions={<button className="primary" onClick={createNew}><Plus size={17}/>New template</button>} />
