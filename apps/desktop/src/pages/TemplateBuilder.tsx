...
function pagePixelWidth(pageSize: 'A4' | 'Letter', orientation: 'Portrait' | 'Landscape') {
  if (pageSize === 'A4') return orientation === 'Portrait' ? 794 : 1123;
  return orientation === 'Portrait' ? 816 : 1056;
}

function defaultTableElementWidth(pageSize: 'A4' | 'Letter', orientation: 'Portrait' | 'Landscape') {
  // Keep a document-safe 30 px inset on both sides while maximizing usable table width.
  return Math.max(430, pagePixelWidth(pageSize, orientation) - 60);
}
...
export function TemplateBuilder({ onNavigate }: { onNavigate: (route: AppRoute) => void }) {
  ...
  function addTableElement(table: TableDefinition) {
    ...
    const tableElement: BuilderElement = { ...defaults, x: 30, table, width: defaultTableElementWidth(pageSize, orientation), height: table.mode === 'custom' ? Math.max(120, table.rows.length * 34) : 180 };
...
function CanvasElement({ item, selected, zoom, record, source, sources, onSelect, onChange }: { item: BuilderElement; selected: boolean; zoom: number; record: ReturnType<typeof activeRecord>; source: ReturnType<typeof activeSource>; sources: BuilderDataState['sources']; onSelect: () => void; onChange: (patch: Partial<BuilderElement>) => void }) {
  ...
  return <div className={`canvas-element ${selected ? 'selected' : ''} element-${item.type}`} style={{ left: item.x, top: item.y, width: item.width, height: item.height, color: item.color, background: item.type === 'shape' ? item.fill : undefined, fontSize: item.fontSize, textAlign: item.textAlign }} onPointerDown={startDrag} onPointerMove={move} onPointerUp={end} onPointerCancel={end}>
    <ElementContent item={item} record={record} source={source} sources={sources} onElementSelect={onSelect} onTableChange={(table) => onChange({ table })} onTableHeightChange={(height) => { if (Math.abs(item.height - height) >= 1) onChange({ height }); }}/>
...
function ElementContent({ item, record, source, sources, onElementSelect, onTableChange, onTableHeightChange }: { item: BuilderElement; record: ReturnType<typeof activeRecord>; source: ReturnType<typeof activeSource>; sources: BuilderDataState['sources']; onElementSelect: () => void; onTableChange: (table: TableDefinition) => void; onTableHeightChange: (height: number) => void }) {
  ...
  if (item.type === 'table' && item.table) {
    ...
    return <TableCanvas table={item.table} record={record} source={tableSource} documentSource={source} onChange={(table) => { onElementSelect(); onTableChange(table); }} onHeightChange={onTableHeightChange}/>;
  }
  ...
  return <div className="table-properties">
    ...
    {cell ? <div className="table-cell-editor">
      ...
      {cell.type === 'text' ? <>
        ...
        {(cell.valueMode ?? (cell.binding ? 'binding' : 'custom')) === 'formula' && <div className="table-formula-editor"><label>Formula<input value={cell.formula ?? ''} placeholder="Quantity * Rate - Discount" onChange={(e) => patchCell({ formula: e.target.value, valueMode: 'formula' })}/></label><p className="table-cell-help">Use imported numeric field names with +, -, *, / and parentheses. Example: Quantity * Rate - Discount.</p>{bindingSource && <div className="formula-field-chips">{bindingSource.fields.filter((field) => field.type === 'number').slice(0, 12).map((field) => <button key={field.name} type="button" className="secondary compact" onClick={() => patchCell({ formula: `${cell.formula ?? ''}${cell.formula ? ' ' : ''}${field.name}`, valueMode: 'formula' })}>{field.label || field.name}</button>)}</div>}</div>}
        <div className="table-data-format-card">
