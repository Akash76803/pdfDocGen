...
import { fitImageToShapePatch } from '../lib/shapeImageFit.ts';
...
function ShapeMediaContent({ item, bound, className = 'shape-media-image', extraStyle }: { item: BuilderElement; bound: unknown; className?: string; extraStyle?: CSSProperties }) {
  ...
  if (src) return <img className={className} src={src} alt="Shape media" style={{ objectFit: item.imageFit ?? 'contain', objectPosition: item.imageObjectPosition ?? 'center', filter: imageElementFilter(item), transform: `scale(${Math.max(100, Math.min(400, item.imageZoomPercent ?? 100)) / 100})`, ...shapeMediaFrameStyle(item), ...(extraStyle??{}) }}/>
...
function ShapeFitImageAction({ selected, onUpdate }: { selected: BuilderElement; onUpdate: (patch: Partial<BuilderElement>) => void }) {
  ...
  return <div className="shape-fit-image-action">
    ...
    <small>{alreadyHasImage ? 'Clip the current image to the exact Shape outline with centered, proportional cover fit.' : 'Choose an image; it will automatically fit inside this Shape.'}</small>
