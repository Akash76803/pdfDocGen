    ...
    <div className={`builder-page ${!leftOpen ? 'left-collapsed' : ''} ${!rightOpen ? 'right-collapsed' : ''}`}>
      ...
      <div className="builder-grid">
        <aside className="builder-left">
          ...
          <div className="page-tree">{pages.map((page) => <button key={page.id} className={page.id === activePageId ? "tree-row selected" : "tree-row"} onClick={() => { setActivePageId(page.id); setSelectedId(null); setActivePreviewPageIndex(0); }}><FileText size={15}/><span>{page.name}</span><small>{page.settings.preset}</small></button>)}</div>
...
function ShapeMediaContent({ item, bound, className = 'shape-media-image', extraStyle }: { item: BuilderElement; bound: unknown; className?: string; extraStyle?: CSSProperties }) {
  ...
  const safeImageRect = useMemo(() => {
    if (className !== 'shape-media-image background' || item.imageFit !== 'contain' || !(item.shapeClipMedia ?? true)) return null;
  ...
  // Cover stays opt-in. One-click Contain is placed in an inscribed area so
  // non-rectangular masks cannot silently cut off photo corners.
  ...
  if (src) return <img className={className} src={src} alt="Shape media" style={{ objectFit: item.imageFit ?? 'contain', objectPosition: item.imageObjectPosition ?? 'center', filter: imageElementFilter(item), transform: `scale(${Math.max(100, Math.min(400, item.imageZoomPercent ?? 100)) / 100})`, ...shapeMediaFrameStyle(item), ...safeImageStyle, ...(extraStyle??{}) }}/>
...
function ShapeFitImageAction({ selected, onUpdate }: { selected: BuilderElement; onUpdate: (patch: Partial<BuilderElement>) => void }) {
  ...
  return <div className="shape-fit-image-action">
    ...
    <small>{alreadyHasImage ? 'Fit the entire image inside this Shape without cropping or distortion. Empty margins may remain.' : 'Choose an image; it will fit entirely inside the Shape without cropping.'}</small>
